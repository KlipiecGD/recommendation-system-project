...
_cfg = config.get_specific_model_config("cb4_genome_tags")
MODEL_NAME: str = _cfg.get("model_name", "CB4_GenomeTags")
...
class CB4GenomeTags(BaseCBModel):
    """
    Content-based recommender using genome + user tags TF-IDF (CB-4).
    Features: G1 (genome, 1128-dim) + G4 (user tags TF-IDF -> SVD).
    ...
    def __init__(
        ...
        """
        Initialise the CB4GenomeTags model.
        Args:
    ...
    def fit(
        ...
        """
        Args:
            movies_enriched (pd.DataFrame): Enriched movies DataFrame (must contain 'user_tags_text').
...
if __name__ == "__main__":
    ...
    model.save(Path("artifacts/cb4_genome_tags"))
