...
_cfg = config.get_specific_model_config("cb6_overview_sbert")
MODEL_NAME: str = _cfg.get("model_name", "CB6_OverviewSBERT")
...
class CB6OverviewSBERT(BaseCBModel):
    """
    Content-based recommender using sentence-transformer overview embeddings (CB-6).

    Features: G5 - dense 384-dim semantic embedding of movie synopsis via
    ...
    def __init__(
        ...
        """
        Initialise the CB6OverviewSBERT model.
        Args:
    ...
    def fit(
        ...
        self.feature_matrix = builder.build_combined(
            [
                (builder.build_overview_sbert(), 1.0),
            ]
        )
...
if __name__ == "__main__":
    ...
    model = CB6OverviewSBERT()
    model.fit(movies_enriched, movie_enc)
    model.save(Path("artifacts/cb6_overview_sbert"))
