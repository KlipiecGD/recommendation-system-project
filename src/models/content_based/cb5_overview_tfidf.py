...
_cfg = config.get_specific_model_config("cb5_overview_tfidf")
MODEL_NAME: str = _cfg.get("model_name", "CB5_OverviewTFIDF")
...
class CB5OverviewTFIDF(BaseCBModel):
    """
    Content-based recommender using TMDB overview TF-IDF (CB-5).
    Features: G5 - TF-IDF on movie synopsis, SVD-reduced.
    Embedding: sparse text embedding via TF-IDF + TruncatedSVD.
    ...
    def __init__(
        ...
        """
        Initialise the CB5OverviewTFIDF model.
        Args:
    ...
    def fit(
        ...
        """
        Args:
            movies_enriched (pd.DataFrame): Enriched movies DataFrame (must contain 'overview').
        ...
        self.feature_matrix = builder.build_combined(
            [
                (builder.build_overview_tfidf(), 1.0),
            ]
        )
...
if __name__ == "__main__":
    ...
    model = CB5OverviewTFIDF()
    model.fit(movies_enriched, movie_enc)
    model.save(Path("artifacts/cb5_overview_tfidf"))
