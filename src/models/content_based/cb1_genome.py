...
_cfg = config.get_specific_model_config("cb1_genome")
MODEL_NAME: str = _cfg.get("model_name", "CB1_Genome")
...
class CB1Genome(BaseCBModel):
    """
    Content-based recommender using genome scores only (CB-1 baseline).
    Features: G1 - 1,128-dim genome tag relevance scores.
    Embedding: raw normalised genome vector.
    ...
    def __init__(
        ...
        """
        Initialise the CB1Genome model.
        Args:
...
if __name__ == "__main__":
    ...
    model.save(Path("artifacts/cb1_genome"))
    ...
    print("\nSimilar to: American Psycho (2000)")
    print("-" * 50)
    for r in model.similar_movies("American Psycho (2000)", n=5):
        print(f"  {r['title']:45s} score: {r['score']:.4f}")

    user_ratings = {
        "Toy Story (1995)": 5.0,
        "Teenage Mutant Ninja Turtles (1990)": 5.0,
        "American Psycho (2000)": 1.0,
    }
    print("\nRecommendations based on user ratings:")
    print("-" * 50)
    for r in model.recommend_from_ratings(user_ratings, n=10):
        print(f"  {r['title']:45s} score: {r['score']:.4f}")

    loaded = CB1Genome.load(Path("artifacts/cb1_genome"), MODEL_NAME)
    print(f"\nLoaded model: {loaded}")
